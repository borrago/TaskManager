...
public class AddTaskCommandHandler(ITaskRepository taskRepository, IMediator mediator) : IRequestHandler<AddTaskCommandInput, AddTaskCommandResult>
{
    ...
    {
        var task = new Domain.ProjectAggregate.Write.Task(command.ProjectId, command.Title, command.Description, command.EndDate, command.Status, command.Priority);
        await _taskRepository.AddAsync(task, cancellationToken);
        await _taskRepository.UnitOfWork.CommitAsync(cancellationToken);

        var @event = new AddedTaskEventInput(task.Id, task.ProjectId, task.Title, task.Description, task.EndDate, task.Status.ToString(), task.Priority.ToString());
